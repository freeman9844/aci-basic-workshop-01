...
K8S_VERSION="$(az aks get-versions \
  --location "$LOCATION" \
  --query "values[?starts_with(version, '1.34.')].version | [0]" \
  --output tsv)"
...
여기서 `K8S_VERSION` 이 비어 있으면 임의로 `1.33` 이나 `1.35` 를 넣지 마세요. Korea Central 에서 실제로 지원되는 `1.34.x` 패치가 반환될 때까지 멈추는 것이 맞습니다.
...
여기서 `--network-plugin azure` 는 필수입니다. 이 워크숍은 Azure CNI가 아닌 AKS 구성을 지원하지 않습니다.
...
| 증상 | 확인할 것 | 확인 명령 | 조치 |
| --- | --- | --- | --- |
| `No supported Kubernetes 1.34.x version` | Korea Central 지원 버전 | `az aks get-versions --location koreacentral --output table` | 다른 버전을 강제로 넣지 말고, `1.34.x` 가 보일 때까지 대기하거나 교육용 구독/지역 상태를 확인 |
