...
- 하나의 AKS 클러스터에서 두 가지 VN2 node path(`ondemand`, `standby`)를 분리해 유지할 수 있습니다.
- 세 가지 hands-on exercise(ondemand, standby uncached, standby cached)를 설명할 수 있으며, cached exercise가 standby virtual node를 다시 사용한다는 점을 이해할 수 있습니다.
- `STANDBY_POOL` 이름과 pool 상태를 다음 모듈까지 그대로 이어 갈 수 있습니다.
- fixed system node에는 benchmark routing label을 추가하지 않고 두 virtual node의 readiness만 확인하는 계약을 유지할 수 있습니다.
...
- `results/workshop.env` 에는 `LOCATION`, `RG`, `VNET`, `AKS_SUBNET`, `CG_SUBNET`, `NAT_NAME`, `NAT_PIP_NAME`, `AKS`, `VM_SIZE`, `AKS_IDENTITY`, `AKS_IDENTITY_ID`, `K8S_VERSION` 이 저장되어 있다.
...
1. Module 02에서 만든 foundation state file을 먼저 source 하고, `$RG`, delegated `cg` subnet, AKS context가 그대로 이어지는지 확인합니다.
2. VN2 Helm chart 저장소를 추가하고 chart version `1.3410.26081102` 와 두 release 이름을 state file에 저장합니다.
3. `vn2-ondemand` 와 `vn2-standby` 를 서로 다른 namespace에 설치하고 standby 쪽은 ready capacity 1을 유지하도록 고정합니다.
...
6. standby pool을 정확히 하나만 찾고 `STANDBY_POOL` 로 export 한 뒤, foundation 12-key state를 16-key state로 원자적으로 확장합니다.
7. 마지막에는 healthy standby pool 과 running 1을 확인하고 다음 모듈에서 재사용할 상태를 점검합니다.
...
- `RG`, `AKS`, `CG_SUBNET` 같은 foundation state를 그대로 다시 사용할 수 있습니다.
...
여기서는 새 benchmark 변수를 만들지 않습니다. Module 02가 남긴 12-key foundation state를 그대로 받아야 이후 Helm install, webhook 확인, pool 조회가 모두 같은 실습 환경을 가리킵니다. `results/workshop.env is the authoritative workshop state` 이므로 fresh Cloud Shell 에서는 먼저 이 파일을 source 한 뒤 같은 확인을 다시 실행합니다.
...
( set -euo pipefail
  ...
  helm upgrade --install vn2-standby virtualnode/virtualnode \
    ...
    --set standbyPool.maxReadyCapacity=1 \
    --set standbyPool.refillPolicy=always \
...
Standby 쪽은 `standbyPool.standbyPoolsCpu=1`, `standbyPool.standbyPoolsMemory=2`, `standbyPool.maxReadyCapacity=1`, `standbyPool.refillPolicy=always` 를 그대로 유지합니다. 이 `1 vCPU / 2 GiB` 프로필이 현재 구독이나 지역 API에서 거부되면 더 큰 값을 자동으로 고르지 말고, 실패 원인과 허용 가능한 최소 프로필을 먼저 확인해야 합니다.
...
`kubectl wait` 는 label 이 아직 안 생긴 짧은 race 구간에서는 바로 끝날 수 있으므로, 위처럼 먼저 label 등장을 최대 10분 동안 bounded polling 한 뒤 Ready wait 로 넘어갑니다. 이 단계가 의미하는 capacity 기준은 16-vCPU/64-GiB fixed system node 한 대가 두 VN2 infrastructure release와 cluster system Pod를 안정적으로 호스팅해야 한다는 것입니다. benchmark workload는 fixed system node에 배치하지 않습니다.
...
Module 03에서 확인하는 경로는 `ondemand` 와 `standby` 두 가지 VN2 node path 뿐입니다. 이후 세 가지 hands-on exercise 중 standby uncached/cached 는 같은 `benchmark-path=standby` virtual node를 재사용하고, 차이는 image cache 적용 여부와 pool recycle 여부로만 만듭니다.
...
이 변수는 다음 모듈에서 그대로 재사용합니다. 이름을 다른 변수로 바꾸지 말고 `STANDBY_POOL` 하나만 유지해야 `run-benchmark.sh`, pool recycle, image cache 단계가 같은 대상을 가리킵니다. Module 03가 끝나면 foundation 12 keys에 `VN2_CHART_VERSION`, `ONDEMAND_RELEASE`, `STANDBY_RELEASE`, `STANDBY_POOL` 이 더해져 총 16개 state keys 가 남아야 하며, 이전 파일 끝에 export 를 덧붙이지 않고 전체 state file 을 원자적으로 다시 써서 키가 각각 정확히 한 번씩만 남게 합니다.

### 7) healthy standby pool 과 running 1 확인
...
이는 standby pool 이 degraded pool 이 아니고, ready standby instance 한 개가 실제로 준비되었다는 뜻입니다. 이후 세 가지 hands-on exercise는 `benchmark-path=ondemand` 한 path와 `benchmark-path=standby` 한 path 위에서 진행하고, cached exercise는 같은 standby virtual node를 재사용합니다.
...
- `results/workshop.env` 가 Module 02 foundation 12 keys를 유지한 채 `VN2_CHART_VERSION`, `ONDEMAND_RELEASE`, `STANDBY_RELEASE`, `STANDBY_POOL` 까지 포함한 16-key state로 다시 저장되었다.
