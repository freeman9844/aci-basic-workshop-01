...
- Cloud Shell 세션이 살아 있어 `kubectl`, `az`, `helm` 명령을 바로 다시 실행할 수 있다.
- `RG` 와 `STANDBY_POOL` 환경 변수를 같은 이름으로 유지하고 있다.
...
| 증상 | 실제 증거 | 확인 명령 | 조치 |
| --- | --- | --- | --- |
| VN2 node 가 `NotReady` 이다 | virtual node 자체보다 먼저 infrastructure Pod/cluster event 를 봐야 한다 | `kubectl get nodes -L benchmark-path -o wide`, `kubectl get pods -A -o wide`, `kubectl get events -A --sort-by=.metadata.creationTimestamp \| tail -n 40` | 8 vCPU/32 GiB headroom, kubelet/Standby Pool RBAC, subnet 설정을 확인한 뒤 `NotReady` 원인을 먼저 제거한다 |
